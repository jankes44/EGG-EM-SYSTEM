...
router.get("/", auth, (req, res) =>
  jwt.verify(req.token, process.env.SECRET_KEY, (err) => {
    if (err) {
      ...
      con.query("SELECT * FROM levels", (err, rows) => res.json(rows));
    }
  })
),
  ...
  router.post("/", auth, function (req, res) {
    jwt.verify(req.token, process.env.SECRET_KEY, (err) => {
      if (err) {
        ...
        con.query(
          "INSERT INTO levels SET `description`=?, `location`=?, `description`=?",
  ...
  jwt.verify(req.token, process.env.SECRET_KEY, (err) => {
    if (err) {
      ...
      if (req.body.description) {
        con.query(
          "UPDATE `levels` SET `group_name`=?, `description`=? where `id`=(?)",
        ...
        con.query(
          "UPDATE `levels` SET `group_name`=? where `id`=(?)",
