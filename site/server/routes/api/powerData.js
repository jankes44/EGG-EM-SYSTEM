...
const rangeQueries = {
  day:
    "select UNIX_TIMESTAMP(date_hour) as epoch, ${values} \
  ...
  week:
    "select UNIX_TIMESTAMP(CONCAT( date, '-', hour*6)) as epoch, ${values} as 'values' \
            from ( \
  ...
  year:
    "select UNIX_TIMESTAMP(week_start) as epoch, ${values} as 'values' from ( \
...
const getValuesInRange = (range_, measure) => {
  const values =
    measure === "voltage"
      ? "voltage"
      : `group_concat(${measure} order by line)`;
  return fillTemplate(rangeQueries[range_], {
    values: values,
    measure: measure,
  });
};
...
router.post("/aggregate", auth, (req, res, next) => {
  if (req.body.measure === "power") {
    ...
    ranges.forEach((range) =>
      byLines.forEach((byLine) => params.push({ range: range, byLine: byLine }))
    );
    console.log(params, params.length);

    const queries = params
      .map((p) => aggregatePower(p.range, p.byLine))
      .join(";");
