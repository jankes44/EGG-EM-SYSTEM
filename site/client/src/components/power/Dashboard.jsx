...
class Dashboard extends React.Component {
  ...
  graphDateFormat = Intl.DateTimeFormat('en', {weekday: 'long', month: 'short', day: 'numeric'})
  ...
  yAxisTitles = {
    "voltage": "Voltage (V)", "power": "Power (W)", "current": "Current (mA)"
  }
  ...
  async fetchData() {
    ...
    axios
      .post(this.state.URL + "/values", {range: this.state.time_range, measure: this.state.show}, {
        headers: this.state.headers,
    })
    .then(res => {
      const data = res.data
      switch(this.state.show){
        case "voltage": {
          const voltage = data.data.map((d) => ({ x: d.epoch, y: d.values }));
    ...
    axios
    .post(this.state.URL + "/aggregate", 
      {measure: "power"},{headers: this.state.headers}
    ).then(res => {
    ...
    if (data_){
      ...
      if (measure === "voltage"){
        ...
        if (this.state.time_range === "day"){
          labels = data_.map((x) => new Date(new Date(x["x"] * 1000))).map(this.dayGraphDateFormat.format)
        } 
        else {
          labels = data_.map((x) => new Date(new Date(x["x"] * 1000))).map(this.graphDateFormat.format)
        }
      }
      else {
        if (this.state.time_range === "day"){
          labels = data_.points.map((x) => new Date(new Date(x["x"] * 1000))).map(this.dayGraphDateFormat.format)
        } 
        else {
          labels = data_.points.map((x) => new Date(new Date(x["x"] * 1000))).map(this.graphDateFormat.format)
        }
        datasets = []
      ...
      graph = <Line ref={this.chartReference} data={data} options={options}/>
    }
    ...
    return (
      <Container>
        ...
        <Grid container direction="row" justify="center" alignItems="center" spacing={4}>
          <SummaryCard title="30 Days" total_data={this.state.summary.month} 
          line_data={this.state.summary.month_line}/>
          <SummaryCard title="3 Months" total_data={this.state.summary.three_months} 
          line_data={this.state.summary.three_months_line}/>
          <SummaryCard title="Year" total_data={this.state.summary.year} 
        ...
        <Grid container direction="row">
          ...
          <FormLabel>Measurement: </FormLabel>
            <RadioGroup name="measurement" row onChange={this.onMeasureChangeValue}>
              {
                this.measurements.map(m => <FormControlLabel 
                  key={m.toLowerCase()} 
                  value={m.toLowerCase()}
                  control={<Radio />}
                  label={m}
            ...
            <FormLabel>Time Range: </FormLabel>
              <RadioGroup name="time_range" row onChange={this.onTimeRangeChangeValue}>
                <FormControlLabel key="day" value="day" control={<Radio/>} label="1 Day" 
                  checked={this.state.time_range === "day"} />
                <FormControlLabel key="month" value="month" control={<Radio/>} label="1 Month" 
                checked={this.state.time_range === "month"} />
                <FormControlLabel key="three_months" value="three_months" control={<Radio/>} label="3 Months" 
                checked={this.state.time_range === "three_months"} />
                <FormControlLabel key="year" value="year" control={<Radio/>} label="1 Year" 
